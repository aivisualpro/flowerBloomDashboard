...
export default function ViewRecipient() {
  ...
  const openConfirm = (row: RecipientRow) => setConfirm({ open: true, id: (row.id ?? row._id) ?? null, name: row.name });
  ...
  const columns: GridColDef<RecipientRow>[] = [
    {
      field: 'image',
      headerName: 'Image',
      width: 110,
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      >
        ...
        <Button isLink href="/recipients/add" isStartIcon startIcon={<IoBag />} variant="primary" color="primary">
          Add Recipient
        </Button>
      </div>
