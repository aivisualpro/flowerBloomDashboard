...
import { Save, ArrowLeft, Upload, Loader2, Trash2, Sparkles } from 'lucide-react';
...
export default function AddOrEditOccasions() {
  const router = useRouter();
  const { id } = useParams();
  const occasionId = Array.isArray(id) ? id[0] : id;
  const isEdit = Boolean(occasionId);
  ...
  // Fetch occasion detail if edit
  const { data: detail, isFetching: isLoadingDetail } = useQuery({
    queryKey: ['occasion', occasionId],
    queryFn: () => getOccasionById(occasionId as string),
    enabled: isEdit && !!occasionId,
    refetchOnWindowFocus: false
  });

  const { mutateAsync: addOccasion, isPending: isAdding } = useAddOccasion();
  const { mutateAsync: updateOccasion, isPending: isUpdating } = useUpdateOccasion();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await updateOccasion({ id: occasionId as string, formData });
      } else {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-900">
                {isEdit ? 'Edit Occasion' : 'Create Occasion'}
              </h1>
            </div>
          </div>
          ...
          <div className="space-y-8 lg:col-span-2">
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>Occasion name and details</CardDescription>
              </CardHeader>
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>Visual representation of the occasion</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                        {form.image ? (
                            <div className="relative h-40 w-40 overflow-hidden rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center">
                                <img src={form.image} alt="Occasion" className="h-full w-full object-cover" />
                                <Button
                                    size="icon"
                                    variant="destructive"
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <p className="text-xs text-muted-foreground">High quality image helps with category selection.</p>
                        </div>
                    </div>
                </div>
              </CardContent>
                ...
                <CardContent className="space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                            ...
                            <p className="text-xs text-muted-foreground">Visible on store frontend</p>
                        </div>
                        <Switch
