...
export default function OccasionsTable() {
  ...
  const delMutation = useDeleteOccasion();

  const filteredData = React.useMemo(() => {
    if (!occasions?.rows) return [];
    return occasions.rows.filter((row: Occasion) => {
  ...
  const handleDelete = async () => {
    if (occasionToDelete) {
      await delMutation.mutateAsync(occasionToDelete.id);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="w-full flex justify-center mb-[-20px]">
          <ParticleTextEffect words={["Special", "Occasions"]} />
      </div>
        ...
        <CardHeader className="p-6 border-b bg-neutral-50/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button onClick={() => router.push('/occasions/add')} className="bg-primary hover:bg-primary/90 text-white shadow-md transition-all rounded-xl">
          ...
          <Table>
            <TableHeader className="bg-neutral-50/50">
              <TableRow className="hover:bg-transparent border-neutral-100">
                <TableHead className="w-[100px] pl-6 py-4 font-semibold text-neutral-600 uppercase text-[10px] tracking-wider">Cover</TableHead>
                <TableHead className="font-semibold text-neutral-600 uppercase text-[10px] tracking-wider">Occasion Name</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                filteredData.map((occ: Occasion) => (
                  <TableRow key={occ.id} className="hover:bg-neutral-50/50 transition-colors border-neutral-100 group">
                    <TableCell className="pl-6 py-4">
                      <div className="h-14 w-14 rounded-xl overflow-hidden border border-neutral-100 shadow-sm bg-neutral-100 flex items-center justify-center">
                        {occ.image ? (
                          <img src={occ.image} alt="" className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
                        ) : (
                    ...
                    <TableCell className="font-semibold text-neutral-800">
                      <div>
                        {occ.name}
                        <div className="text-xs text-neutral-400 font-normal">{occ.ar_name}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>{getStatusBadge(occ.isActive)}</TableCell>
                    <TableCell className="text-right pr-6">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button
                                ...
                                onClick={() => router.push(`/occasions/edit/${occ.id}`)}
                            >
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="rounded-2xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Delete Occasion</DialogTitle>
            <DialogDescription className="text-neutral-500 pt-2 text-sm leading-relaxed">
              Are you sure you want to delete <span className="font-bold text-neutral-900">{occasionToDelete?.name}</span>?
